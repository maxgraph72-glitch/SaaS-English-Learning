...
'True after a missed due date has reduced the stage and until the recovery review is submitted.';
...
begin
  ...
  set current_group = 'repeat'::public.vocabulary_group,
  ...
  where item.user_id = v_user_id
    and item.repetition_stage between 1 and 5
...
begin
  ...
  if v_item.next_review_date < v_review_date
     and not v_item.overdue_stage_decay_pending then
    v_item.current_group := 'repeat'::public.vocabulary_group;
    v_item.repetition_stage :=
      greatest(v_item.repetition_stage - 1, 1)::smallint;
    v_item.overdue_stage_decay_pending := true;
  end if;
  ...
  if not p_correct then
    ...
    v_stage_after := 1;
    v_interval := 1;
  elsif p_response_time_ms < 3000
        and v_item.overdue_stage_decay_pending then
